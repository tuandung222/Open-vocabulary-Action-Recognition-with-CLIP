...
class ModelLoader:
    ...
    def load_mlflow_model(
        model_uri: str, device: Optional[str] = None
    ) -> CLIPLabelRetriever:
        """
    ...
    def load_onnx_model(model_path: str) -> Any:
        ...
        try:
            ...
            raise ImportError(
                "ONNX Runtime not installed. Install with: pip install onnxruntime-gpu or onnxruntime"
            )

    @staticmethod
    def load_tensorrt_model(model_path: str) -> Tuple[Any, Dict]:
        """
...
class InferencePipeline:
    ...
    def _load_model(self):
        ...
        if self.model_type == "pytorch":
            ...
            self.model = ModelLoader.load_mlflow_model(
                model_uri=self.model_path, device=self.device
            )
            # Get labels from model
            self.labels = self.model.labels

        elif self.model_type == "onnx":
            self.onnx_session = ModelLoader.load_onnx_model(model_path=self.model_path)

        elif self.model_type == "tensorrt":
            (
                self.tensorrt_context,
                self.tensorrt_binding_info,
            ) = ModelLoader.load_tensorrt_model(model_path=self.model_path)
    ...
    def _pytorch_inference(self, processed_image: torch.Tensor) -> Dict[str, Any]:
        """
        ...
        with torch.no_grad():
            ...
            result = {
                ...
                "visual_embeds": visual_embeds.tolist()
    ...
    def _onnx_inference(self, processed_image: np.ndarray) -> Dict[str, Any]:
        """
    ...
    def _tensorrt_inference(self, processed_image: np.ndarray) -> Dict[str, Any]:
        """
        ...
        self.tensorrt_context.execute_async_v2(
            bindings=bindings, stream_handle=self.cuda_stream.handle
        )
            ...
            if frame_idx % frame_interval == 0:
                ...
                try:
                    ...
                    if frame_idx % (frame_interval * 10) == 0:
                        logger.info(
                            f"Processed frame {frame_idx}/{frame_count} ({frame_idx/frame_count*100:.1f}%)"
                        )
    ...
    def process_directory(
        ...
        for i, image_path in enumerate(image_files):
            try:
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        f"Processed {i+1}/{len(image_files)} images ({(i+1)/len(image_files)*100:.1f}%)"
                    )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run inference pipeline for HAR classification"
    )
    ...
    input_group.add_argument("--directory", type=str, help="Path to input directory")
...
def main():
    ...
    try:
        ...
        if args.image:
            ...
            print(
                f"Top classes: {list(zip(result['top_classes'], [f'{s:.4f}' for s in result['top_scores']]))}"
            )
