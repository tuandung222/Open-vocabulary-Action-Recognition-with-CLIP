...
from CLIP_HAR_PROJECT.evaluation.evaluator import (
...
from CLIP_HAR_PROJECT.utils.distributed import is_main_process, get_rank, get_world_size
from CLIP_HAR_PROJECT.utils.mlflow import log_params, log_metrics, log_model, log_artifact
...
class TrainingPipeline:
        ...
        **kwargs
        ...
        if self.is_main_process and (use_mlflow or use_wandb):
            self.tracker = create_tracker(
                ...
                mlflow_tracking_uri=self.config.mlflow.tracking_uri if hasattr(self.config, "mlflow") else None,
                mlflow_artifacts_dir=self.config.mlflow.artifacts_dir if hasattr(self.config, "mlflow") else "./mlruns",
                wandb_project=self.config.wandb.project_name if hasattr(self.config, "wandb") else "clip_har",
                wandb_group=self.config.wandb.group_name if hasattr(self.config, "wandb") else None,
                wandb_entity=self.config.wandb.entity if hasattr(self.config, "wandb") else None,
                config={
                    "model": self.config.model.__dict__ if hasattr(self.config, "model") else {},
                    "training": self.config.training.__dict__ if hasattr(self.config, "training") else {},
                    "data": self.config.data.__dict__ if hasattr(self.config, "data") else {}
                }
            )
            # Start the tracking run
            self.tracker.start_run(run_name=f"{self.experiment_name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def prepare_data(
        ...
        self.image_processor = CLIPImageProcessor.from_pretrained(self.config.model.model_name)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.is_main_process and self.tracker:
            ...
            self.tracker.log_params({
                ...
                "trainable_param_ratio": num_trainable_params / num_params if num_params > 0 else 0,
        ...
        if self.is_main_process and self.tracker and results:
            ...
            if hasattr(self.trainer, "best_model_path") and os.path.exists(self.trainer.best_model_path):
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        if self.is_main_process and self.tracker:
            # Log metrics
            self.tracker.log_metrics({f"test_{k}": v for k, v in eval_metrics.items() 
        ...
        if "onnx" in formats:
            try:
                ...
                onnx_path = export_to_onnx(
                    ...
                    input_shape=(1, 3, self.config.data.image_size, self.config.data.image_size),
                    dynamic_axes=True
                )
                ...
                is_valid = validate_exported_model(
                    onnx_path, 
                    shape=(1, 3, self.config.data.image_size, self.config.data.image_size)
                )
                logger.info(f"ONNX model validation: {'passed' if is_valid else 'failed'}")
            ...
            if self.is_main_process and self.tracker:
                ...
                for format_name, result in benchmark_results.items():
                    self.tracker.log_metrics({
                        f"benchmark_{format_name}_latency_ms": result["avg_latency_ms"],
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run training pipeline for HAR classification")
    
    # Pipeline configuration
    parser.add_argument("--config_path", type=str, default=None,
                        help="Path to configuration file")
    parser.add_argument("--distributed_mode", type=str, default="none", 
