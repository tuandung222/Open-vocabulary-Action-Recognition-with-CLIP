...
class DistributedTrainer:
    ...
    def __init__(
        ...
        self.config = config if isinstance(config, TrainingConfig) else TrainingConfig.from_dict(config)
        self._logger = logger
    ...
    def setup_output_dir(self):
        ...
        prefix = f"{output_dir}/{self.project_name}/{self.group_name}/{self.experiment_name}"
    ...
    def setup_optimizers_before_training(self):
        ...
        self.scheduler = transformers.get_cosine_schedule_with_warmup(
            ...
            num_training_steps=self.config.get("max_epochs", 10) * len(self.train_dataloader),
    ...
    def train(self) -> Dict[str, Any]:
        ...
        while True:  # Continue until max epoch or patience is reached
            ...
            pbar_train_dataloader.set_description(f"Epoch {self.cur_epoch} (Rank {self.distributed_config.rank})")
            ...
            for data in pbar_train_dataloader:
                ...
                if (self.cur_epoch >= self.config.max_epochs or self.exit_by_patience):
                    print(f"Exit requirement reached, exiting (Rank {self.distributed_config.rank})")
                ...
                with torch.autocast(
                    ...
                    dtype=torch.bfloat16 if self.config.mixed_precision else torch.float32
                ):
                    # Run model
                ...
                if self.cur_step % self.config.train_log_interval == 0 and is_main_process(self.distributed_config):
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        if self.distributed_config.mode != DistributedMode.NONE:
            ...
            gather_pred = [torch.zeros_like(predictions_tensor) for _ in range(world_size)]
            gather_ref = [torch.zeros_like(references_tensor) for _ in range(world_size)]
    ...
    def handle_checkpoint_with_patience(self, metrics: Dict[str, Any], set_patience=True):
    ...
    def log(self, name: str, value: Union[torch.Tensor, float, int]):
        ...
        if self._logger is not None and is_main_process(self.distributed_config):
            self._logger.log(
                {name: value, "epoch": self.cur_epoch},
                step=self.cur_step
            )
